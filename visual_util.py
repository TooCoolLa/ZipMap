...
def predictions_to_glb(
    ...
    """
    Converts ZipMap predictions to a 3D scene represented as a GLB file.
            ...
            for i, image_name in enumerate(image_list):
                ...
                if os.path.exists(mask_filepath):
                    ...
                    if skyseg_session is None:
                        ...
                        skyseg_session = onnxruntime.InferenceSession("skyseg.onnx", sess_options=sess_opts, providers=providers)
    ...
    if show_cam:
        ...
        for i in range(num_cameras):
            ...
            integrate_camera_into_scene(scene_3d, camera_to_world, current_color, scene_scale * cam_size_factor)

    # Align scene to the observation of the first camera, centered on the point cloud
    scene_center = np.median(vertices_3d, axis=0) if vertices_3d.shape[0] > 0 else None
    scene_3d = apply_scene_alignment(scene_3d, extrinsics_matrices, scene_center=scene_center, scene_scale=scene_scale)
...
def apply_scene_alignment(scene_3d: trimesh.Scene, extrinsics_matrices: np.ndarray, scene_center: np.ndarray = None, scene_scale: float = None) -> trimesh.Scene:
    """
    ...
    Args:
        ...
        scene_center (np.ndarray, optional): 3D centroid of the point cloud to center the scene on.
        scene_scale (float, optional): Diagonal of the 5th-95th percentile bounding box, used to
            normalize large scenes so the Gradio viewer's fixed camera radius stays outside the scene.
    ...
    # Center the scene on the point cloud centroid so the viewer's initial camera
    # focuses on the actual scene content rather than the first camera's position.
    # This is especially important for linear trajectories (e.g., walkthroughs) where
    # the first camera is at one end and the centroid is far from the origin.
    if scene_center is not None:
        rotation = initial_transformation[:3, :3]
        initial_transformation[:3, 3] = -rotation @ scene_center

    # Normalize large scenes so the Gradio viewer (default camera radius ≈ 3) stays
    # outside the scene. scene_scale was already computed but only applied to camera
    # frustum sizes — here we use it to also scale the scene geometry down uniformly.
    # Camera frustums (sized by scene_scale * cam_size_factor) shrink proportionally
    # and remain visually correct relative to the scene.
    _target_scale = 4.0
    if scene_scale is not None and scene_scale > _target_scale:
        normalize_factor = _target_scale / scene_scale
        scale_matrix = np.diag([normalize_factor, normalize_factor, normalize_factor, 1.0])
        initial_transformation = scale_matrix @ initial_transformation
