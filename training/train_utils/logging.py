...
def _format_param_count(count: int) -> str:
    """Return a human-readable representation of a parameter count."""
...
def setup_logging(
    ...
    # Also configure the root logger's level to ensure all child loggers inherit it
    logging.root.setLevel(log_level_primary)

    # Configure the actual root logger to have the same handlers
    # so that logging.info() calls from any module will work
    root_logger = logging.getLogger()
    root_logger.setLevel(log_level_primary)
