...
def safe_mean(tensor, default_value=0.0):
    """Safely compute mean of tensor, handling empty or invalid values."""
    if tensor.numel() == 0:
        logging.warning("Empty tensor passed to safe_mean. Returning default value.")
        return torch.tensor(default_value, device=tensor.device, dtype=tensor.dtype)
    ...
    if valid_count > 0:
        if valid_count < tensor.numel():
            logging.warning(f"Tensor passed to safe_mean contains {tensor.numel() - valid_count} invalid (NaN/Inf) values. Ignoring them in mean computation.")
        return tensor[valid_mask].sum() / valid_count
    else:
        return torch.tensor(default_value, device=tensor.device, dtype=tensor.dtype)
