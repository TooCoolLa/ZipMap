...
class ComposedDataset(Dataset, ABC):
    ...
    def __init__(self, dataset_configs: dict, common_config: dict, **kwargs):
        ...
        self.disable_all_augs = common_config.augs.disable_all_augs
    ...
    def __getitem__(self, idx_tuple):
        ...
        # # Invalidate all points if fir`st frame has no valid points
        # if point_masks.numel() > 0 and point_masks[0].sum() == 0:
        #     point_masks[:] = False

        if self.disable_all_augs:
            # Skip all augmentations if disabled
            pass
        else:
