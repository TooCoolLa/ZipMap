...
class DynamicBatchSampler(Sampler):
    ...
    def __init__(self,
        ...
        self.image_num_rng = random.Random()
    ...
    def set_epoch(self, epoch):
        ...
        self.sampler.set_epoch(epoch)
        self.epoch = epoch
        # get the local rank for seeding
        if dist.is_available() and dist.is_initialized():
            local_rank_for_sample = int(os.environ["LOCAL_RANK"]) % 8
        else:
            local_rank_for_sample = 0
        self.image_num_rng.seed(epoch * 100 + local_rank_for_sample)
    ...
    def __iter__(self):
        ...
        while True:
            try:
                ...
                # random_image_num = int(self.rng.choices(self.possible_nums, weights=self.normalized_weights)[0])
                random_image_num = int(self.image_num_rng.choices(self.possible_nums, weights=self.normalized_weights)[0])
