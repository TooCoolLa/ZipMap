...
def crop_image_depth_and_intrinsic_by_pp(
    ...
    if strict:
        if (image.shape[:2] != target_shape).any():
            ...
            logging.info(f"{filepath} does not meet the target shape")
...
def depth_to_world_coords_points(
    ...
    Args:
        ...
        extrinsic (np.ndarray):
            Extrinsic matrix of shape (3, 4), representing the camera pose in OpenCV convention (world-to-camera).
        intrinsic (np.ndarray):
...
def save_ply(points, colors, filename):
    ...
    if points_visual_rgb.dtype == np.uint8:
        # normalize to [0, 1]
        points_visual_rgb = points_visual_rgb.astype(np.float32) / 255.0
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(points_visual.astype(np.float32))
    pcd.colors = o3d.utility.Vector3dVector(points_visual_rgb.astype(np.float32))
