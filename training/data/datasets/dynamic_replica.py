...
class DynamicReplicaDataset(BaseDataset):
    def __init__(
        ...
        common_conf,
        split: str = 'train',
        DYNAMIC_REPLICA_DIR: str = "/path/to/dynamic_replica/",
        ...
        self.is_metric = True  # Dynamic Replica is metric
        logging.info(f"DYNAMIC_REPLICA_DIR is {self.DYNAMIC_REPLICA_DIR}")
    ...
    def _load_data(self):
        ...
        metadata_file = osp.join(self.DYNAMIC_REPLICA_DIR, "all_scenes_dict.npz")
        ...
        assert self.split in all_scenes_npz, f"Split {self.split} not found in {metadata_file}"
        ...
        logging.info(f"Dynamic Replica loading {self.split} metadata from precomputed {metadata_file}")
        ...
        for scene_name in sorted(all_scenes_dict.keys()):
            ...
            # comment out the line below if used the latest dynamic_replica preprocessing script
            sequence_trajectories_w2c = closed_form_inverse_se3(sequence_trajectories)
            sequence_trajectories_w2c[:, :3, :3] = sequence_trajectories_w2c[:, :3, :3].transpose(0, 2, 1)  # Convert to opencv format
            sequence_trajectories = closed_form_inverse_se3(sequence_trajectories_w2c)
            # end
        ...
        logging.info(f"Dynamic Replica {self.split} data loaded: {len(self.sequence_list)} scene trajectories")
    ...
    def _show_stats(self):
        ...
        plt.title(f"Dynamic Replica {self.split.capitalize()} Dataset: Number of Images per Sequence")
        plt.tight_layout()
        plt.savefig(f"dynamic_replica_{self.split}_image_count_histogram.png")
    ...
    def get_data(
        ...
        # Dynamic Replica is video-like; use fixed-interval sampling
        ...
        for image_idx in ids:
            ...
            scene_dir = osp.join(self.DYNAMIC_REPLICA_DIR, self.split, scene_path, "left")
            ...
            depth_map = threshold_depth_map(depth_map, min_percentile=-1, max_percentile=-1)
