...
class ARKitScenesDataset(BaseDataset):
    ...
    def get_data(
        ...
        if np.random.choice([True, False]):
            ...
            ordered_video = False
            image_idxs = self.groups[seq_index]
            image_idxs = np.random.permutation(image_idxs)
            if len(image_idxs) > img_per_seq:
                ids = image_idxs[:img_per_seq]
            else:
                if np.random.random() < 0.8:
                    ids = np.random.choice(image_idxs, size=img_per_seq, replace=True)
                else:
                    repeat_num = img_per_seq // len(image_idxs) + 1
                    ids = np.tile(image_idxs, repeat_num)[:img_per_seq]
