...
class SpringDataset(BaseDataset):
    def __init__(
        ...
        self.is_metric = True  # Spring is metric
        self.video_prob = video_prob
        logging.info(f"SPRING_DIR is {self.SPRING_DIR}")
    ...
    def _load_data(self):
        ...
        metadata_file = osp.join(self.SPRING_DIR, "all_scenes_dict.npz")
        ...
        logging.info(f"Spring loading metadata from precomputed {metadata_file}")
        ...
        logging.info(f"Spring {self.split} data loaded: {len(self.sequence_list)} scene trajectories")
    ...
    def _show_stats(self):
        ...
        plt.title(f"Spring {self.split.capitalize()} Dataset: Number of Images per Sequence")
        plt.tight_layout()
        plt.savefig(f"spring_{self.split}_image_count_histogram.png")
    ...
    def get_data(
        ...
        # Spring is video-like; use fixed-interval sampling
        ...
        for image_idx in ids:
            ...
            scene_dir = osp.join(self.SPRING_DIR, scene_path)
            ...
            # Process depth map (from original: only invalid check)
