...
SELECTED_SCENE_NAMES = [
    ...
    "AmusementPark",
    "AncientTowns",
    # "Antiquity3D", # too dark
    "Apocalyptic",
    # "ArchVizTinyHouseDay", # window outdoor too weird
...
class TartanAirV2Dataset(BaseDataset):
    def __init__(
        self,
        common_conf,
        split: str = 'train',
        TartanAirV2_DIR: str = "/home/haian/Dataset/tartanairv2/",
        ...
        self.TartanAirV2_DIR = TartanAirV2_DIR
        self.max_interval = max_interval
        self.split = split
        self.min_num_images = min_num_images
        logging.info(f"TartanAirV2_DIR is {self.TartanAirV2_DIR}")
    ...
    def _load_data(self):
        assert os.path.exists(osp.join(self.TartanAirV2_DIR, "all_scenes_dict.npz")), f"all_scenes_dict.npz not found in {self.TartanAirV2_DIR}, please run save_tartanair_v2_metadata.py first."
        all_scenes_dict = np.load(osp.join(self.TartanAirV2_DIR, "all_scenes_dict.npz"), allow_pickle=True)
        ...
        logging.info("TartanAirV2 loading data from precomputed all_scenes_dict.npz")
        ...
        for scene in scene_dirs:
            ...
            for mode in ["Data_easy", "Data_hard"]:
                ...
                mode_dir = os.path.join(self.TartanAirV2_DIR, scene, mode)
        ...
        logging.info(f"TartanAirV2 {self.split} data loaded: {len(self.images)} images from {len(self.scenes)} sequences")

    def _show_stats(self):
        logging.info(f"TartanAirV2 {self.split} dataset statistics:")
    ...
    def get_data(
        ...
        for image_idx in ids:
            ...
            # Load depth map
            depth_filepath = osp.join(scene_dir, basename + "_depth.png")
            
            try:
                image = imread_cv2(image_filepath)
                depth_map = read_decode_depth(depth_filepath)

                # Process depth map similar to original TartanAir processing
                depth_map = np.nan_to_num(depth_map, nan=0, posinf=0, neginf=0)
                depth_map = threshold_depth_map(
                    depth_map, min_percentile=0.1, max_percentile=99, max_depth=80 # sky mask
                )
        ...
        set_name = "tartanairV2"

        batch = {
            "seq_name": set_name + "_" + seq_name,
            "dataset_name": "tartanairv2",
            "ids": np.array(ids),
