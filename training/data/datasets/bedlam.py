...
class BEDLAMDataset(BaseDataset):
    def __init__(
        ...
        self.is_metric = True  # BEDLAM is metric
        self.video_prob = video_prob
        logging.info(f"BEDLAM_DIR is {self.BEDLAM_DIR}")
    ...
    def _load_data(self):
        ...
        metadata_file = osp.join(self.BEDLAM_DIR, "all_scenes_dict.npz")
        ...
        logging.info(f"BEDLAM loading metadata from precomputed {metadata_file}")
        ...
        logging.info(f"BEDLAM {self.split} data loaded: {len(self.sequence_list)} scene trajectories")
    ...
    def _show_stats(self):
        ...
        plt.title(f"BEDLAM {self.split.capitalize()} Dataset: Number of Images per Sequence")
        plt.tight_layout()
        plt.savefig(f"bedlam_{self.split}_image_count_histogram.png")
    ...
    def get_data(
        ...
        # BEDLAM is video-like; use fixed-interval sampling
        ...
        for image_idx in ids:
            ...
            scene_dir = osp.join(self.BEDLAM_DIR, scene_path)
            ...
            # Process depth map (from original: invalid check and > 200m threshold)
            depth_map[~np.isfinite(depth_map)] = 0  # invalid
            depth_map[depth_map > 200.0] = 0.0  # BEDLAM uses 200m threshold
