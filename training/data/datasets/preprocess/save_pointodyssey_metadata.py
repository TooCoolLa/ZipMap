...
def fetch_and_save_pointodyssey_metadata(POINTODYSSEY_DIR, output_file, num_workers=None):
    """
    Process PointOdyssey dataset and save metadata for all splits in a single file

    Args:
        POINTODYSSEY_DIR: Root directory of PointOdyssey dataset
    ...
    for split in ["train", "test", "val"]:
        split_dir = osp.join(POINTODYSSEY_DIR, split)
        ...
        process_func = partial(process_single_scene, POINTODYSSEY_DIR=POINTODYSSEY_DIR, split=split)
        ...
        logging.info(f"PointOdyssey {split} processing complete: {processed_scenes} scenes processed, {skipped_scenes} scenes skipped")
...
def main():
    parser = argparse.ArgumentParser(description="Process PointOdyssey dataset and save metadata")
    parser.add_argument("--input_dir", type=str, default="/home/storage-bucket/haian/zipmap_data/processed_point_odyssey", help="Input PointOdyssey directory")
    ...
    fetch_and_save_pointodyssey_metadata(args.input_dir, args.output_file, num_workers=args.num_workers)
