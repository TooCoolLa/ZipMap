...
class Co3dDataset(BaseDataset):
    def __init__(
        ...
        # Check for merged annotation file
        merged_annotation_path = osp.join(self.CO3D_ANNOTATION_DIR, "co3d_merged_annotations.npz")
        
    
        logging.info(f"CO3D loading merged annotation file: {merged_annotation_path}")
        merged_data_npz = np.load(merged_annotation_path, allow_pickle=True)
        
        # Optimized lazy loading - only load needed categories and splits
        for c in category:
            for split_name in split_name_list:
                file_key = f"{c}_{split_name}"
                if file_key in merged_data_npz.files:
                    # Only deserialize the specific category/split we need
                    annotation = merged_data_npz[file_key].item()
                    ...
                    logging.warning(f"Missing annotation for {file_key}")
        self.sequence_list = list(self.data_store.keys())
        self.sequence_list_len = len(self.sequence_list)
        self.total_frame_num = total_frame_num

        status = "Training" if self.training else "Testing"
        logging.info(f"{status}: Co3D sequence size: {self.sequence_list_len}, total frames: {self.total_frame_num}")
    ...
    def get_data(
        ...
        category = annos[0]["filepath"].split("/")[0]
        for anno in annos:
            ...
            if self.load_depth:
                ...
                mvs_mask = cv2.imread(mvs_mask_path, cv2.IMREAD_GRAYSCALE) > 127
        ...
        batch = {
            "seq_name": set_name + "_" + category + "_" + seq_name,
            "dataset_name": "co3d",
            "ids": ids,
    ...
    def _show_stats(self):
        logging.info(f"CO3D {self.split} dataset statistics:")
        logging.info(f"  Number of sequences: {len(self.sequence_list)}")
        logging.info(f"  Total number of frames: {self.total_frame_num}")
        
        # Statistics on sequence level
        sequence_frame_counts = []
        for seq_name in self.sequence_list:
            frame_count = len(self.data_store[seq_name])
            sequence_frame_counts.append(frame_count)
        
        if sequence_frame_counts:
            min_count = min(sequence_frame_counts)
            max_count = max(sequence_frame_counts)
            avg_count = sum(sequence_frame_counts) / len(sequence_frame_counts)
            logging.info(f"  Frame count per sequence: min={min_count}, max={max_count}, avg={avg_count:.2f}")
